...
.controller('SignUpCtrl', [
    ...
    function($scope, $rootScope, $firebaseAuth, $window) {
        ...
        $scope.createUser = function() {
            var email = this.user.email;
            var password = this.user.password;
			console.log((new Date(2016, 04, 29, 20, 05)).getTime());
            if (!email || !password) {
                $rootScope.notify("Please enter valid credentials");
                return false;
            }
...
.controller('SocialSignInCtrl',['$scope', '$rootScope', '$firebaseAuth', '$window', function($scope, $rootScope, $firebaseAuth, $window) {
	
  	
	$rootScope.login = function(authMethod) {
    $rootScope.authRef.$authWithOAuthRedirect(authMethod).then(function(authData) {
      console.log(authData);
    }).catch(function(error) {
      if (error.code === 'TRANSPORT_UNAVAILABLE') {
        $rootScope.authRef.$authWithOAuthPopup(authMethod).then(function(authData) {
  ...
  $rootScope.authRef.$onAuth(function(authData) {
    if (authData === null) {
